/*
  ...
  Hologram (read_digital), this sketch will send a JSON-encoded list of
  all six digital measurements.
  After uploading the code, open your Hologram dashboard, select  your
  device and send a message matching "read_digital" to the device. 
  
  Shortly after this the sketch should respond with a JSON-encoded list 
  of all six digital measurements. This JSON string can be used for Hologram 
  ...
  accurate readings of the digital pins. We recommend cutting this
...
String HOLOGRAM_DIGITAL_TOPIC = "DIGITAL";
...
// Pair digital pins up with a JSON identifier string
#define DIGITAL_PINS_TO_READ 6
int readPins[DIGITAL_PINS_TO_READ] = {13};
String pinNames[DIGITAL_PINS_TO_READ] = {"13"};

// loop flags to check:
boolean doSendDigital = false; // Send digital values
...
void processSocketRead(int socket, String response) {
  ...
  if (response == "read_digital") {
    doSendDigital = true; // If found set a flag to send digital values
  } else {
...
void setup() {
  ...
  for (int i=0; i<DIGITAL_PINS_TO_READ; i++) {
...
void loop() {
  ...
  // If sendDigital flag is set, do it and clear flag
  if (doSendDigital) {
...
// Compile digital reads into a JSON-encoded string, then send to Hologram
void sendDigitalValues(void) {
  String toSend = "{";
  for (int i = 0; i < DIGITAL_PINS_TO_READ; i++) {
    toSend += "\\\"" + pinNames[i] + "\\\": ";
    toSend += String(digitalRead(readPins[i]));
    if (i < 5) toSend += ", ";
  }
  toSend += "}";
...
void sendHologramMessage(String message) {
  ...
  String hologramMessage;
    ...
    hologramMessage = "{\"k\":\"" + HOLOGRAM_DEVICE_KEY + "\",\"d\":\"" +
        message + "\",\"t\":[\"" + HOLOGRAM_DIGITAL_TOPIC + "\"]}";
